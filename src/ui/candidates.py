...
def view_candidate_detail(conn:duckdb.DuckDBPyConnection):
    if st.checkbox("view potential vendor customer parents"):
        ...
        if selected["selection"]["rows"] != []:
            selected_row_index = selected["selection"]["rows"][0]
            row_dict = df.iloc[selected_row_index].to_dict()
            selected_candidate = SelectedCandidate(**row_dict)
